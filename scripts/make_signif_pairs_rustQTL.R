...
if (length(args) < 3) {
  cat("Usage: Rscript make_signif_pairs_rustQTL.R <threshold_file> <nominal_file> <output_file>\n")
  quit(status = 1)
}
...
required_thr <- c("phenotype_id", "pval_nominal_threshold", "qval")
...
if (ncol(nom) != 10) {
  stop("Expected 10 columns in rust-fastqtl nominal file, found ", ncol(nom))
}
